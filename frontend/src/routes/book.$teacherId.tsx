...
import api, { normalizeTeacher, unwrap, type NormalizedTeacher } from "@/lib/api";
...
const FALLBACK: NormalizedTeacher = {
  id: "1",
...
const SLOTS: Array<{ label: string; hour24: number }> = [
...
function BookPage() {
  ...
  const [teacher, setTeacher] = useState<NormalizedTeacher>(FALLBACK);
  ...
  useEffect(() => {
    api
      ...
      .then((r) => {
        const t = normalizeTeacher(unwrap(r));
        if (t) {
          setTeacher(t);
  ...
  const confirm = async () => {
    ...
    setError(null);
    try {
      const durationMin = type === "trial" ? 30 : 60;
      const startHour = slot.hour24;
      const start = `${pad(startHour)}:00:00`;
      const endTotalMin = startHour * 60 + durationMin;
      const endHour = Math.floor(endTotalMin / 60);
      const endMin = endTotalMin % 60;
      const end = `${pad(endHour)}:${pad(endMin)}:00`;
    ...
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          A Google Meet link has been sent to your email and is available in your dashboard.
        </p>
        <div className="mt-6 flex justify-center gap-2">
          <Link
            to="/dashboard/student"
            className="rounded-md bg-primary px-5 py-2.5 text-primary-foreground font-semibold"
          >
            Go to Dashboard
          </Link>
          <button
            onClick={() => navigate({ to: "/teachers" })}
            className="rounded-md border border-input px-5 py-2.5"
          >
            Find more teachers
          </button>
        </div>
      ...
      <div className="mt-6 space-y-6">
            ...
            {(["trial", "regular"] as const).map((t) => (
              ...
              >
                <p className="font-semibold capitalize">
                  {t === "trial" ? "Free Trial" : "Regular Session"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  {t === "trial" ? "30 mins · Free" : `60 mins · Rs. ${teacher.hourly_rate}`}
                </p>
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(teacher.subjects ?? []).map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm border ${subject === s ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
                {s}
              </button>
            ))}
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {SLOTS.map((s) => (
              <button
                key={s.label}
                onClick={() => setSlot(s)}
                className={`rounded-md border py-2 text-sm ${slot.label === s.label ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
                {s.label}
              </button>
            ))}
        ...
        <Section title="5. Confirm">
          <div className="rounded-lg bg-surface p-4 text-sm space-y-1">
            ...
            <Row k="Date & time" v={`${date} at ${slot.label}`} />
            <Row k="Type" v={type === "trial" ? "Free Trial" : "Regular"} />
            <div className="border-t border-border pt-2 mt-2 flex justify-between font-semibold">
              <span>Total</span>
              <span>{price === 0 ? "Free" : `Rs. ${price.toLocaleString()}`}</span>
            </div>
          </div>
        </Section>
